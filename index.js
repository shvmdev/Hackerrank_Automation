...
const {email,password} = require("./cred");
const fs = require("fs");
// module.exports = {
//     password: "",
//     email: ""
...
async function fn() {
	...
	await tab.type("input[type='text']", email, { delay: 20 });
	await tab.type("input[type='password']", password, { delay: 20 });
...
async function waitAndClickQuestion(name, tab) {
	...
	function findAndClick(name) {
		let allQuestions = document.querySelectorAll(
			".challenges-list .challengecard-title"
		);
		// return idx
		let idx;
		let textContent = [];
